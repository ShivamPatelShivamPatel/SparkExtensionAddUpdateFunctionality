...
abstract class Optimizer(catalogManager: CatalogManager)
  ...
  def nonExcludableRules: Seq[String] =
    EliminateDistinct.ruleName ::
      ...
      OptimizeCaseWhen.ruleName :: Nil
// scala-style:off
  /**
   *  Where possible, Optimize Case When Else statements
   */

  object OptimizeCaseWhen extends Rule[LogicalPlan] {
    import org.apache.spark.sql.catalyst.util.toPrettySQL
    // verify that fa is indeed a function of a, if not return false
    private def faIsAFunctionOfA(fa: Expression, a: Expression): Boolean = {
      val aStr = toPrettySQL(a)
      var result: Boolean = false
      for(i <- fa.children.indices){ //if fa == f(a) then a is in children(fa)   
        if(toPrettySQL(fa.children(i)).contains(aStr))
          result = true
      }
    ...
    // the 2 exlusion conditions for Type1 & Type2 can be handled by checking the children of X or C to verify there is no 'a'
    private def aIsNotInExpr(expr: Expression, a: Expression): Boolean = {
      val aStr = toPrettySQL(a)
      var result: Boolean = true
      for(i <- expr.children.indices){
        if(toPrettySQL(expr.children(i)).contains(aStr))
          result = false  
      }
    ...
    // logical and all the conditions together to see if further processing needs to be done
    private def checkConds(C: Expression, fa: Expression, X: Expression, a: Expression): Boolean = {
      faIsAFunctionOfA(fa, a) && // check fa is function of a by searching for a in fa
      aIsNotInExpr(X, a) && // for X and C search in them and verify a is not in their children
      aIsNotInExpr(C, a) 
    }

    /*
     *  Type 1: CASE WHEN C THEN f(a) + X ELSE a END 
     *       -> CASE WHEN C THEN f(a) ELSE a END + CASE WHEN C THEN X ELSE 0 END
     *  Conditions: a is not mentioned in C, X does not contain a.
     */
   
    /*
     *  Type 2: CASE WHEN C THEN f(a)*X ELSE a END 
     *       -> CASE WHEN C THEN f(a) ELSE a END * CASE WHEN C THEN X ELSE 1 END
     *  Conditions: a is not mentioned in C, X does not contain a.
     */

    def apply(plan: LogicalPlan): LogicalPlan = plan.transformAllExpressions {
      case cw @ CaseWhen(branches, elseValue) => {
        val C = branches.head._1
        val faX = branches.head._2 // faX because we dont know if its faPlusX, faTimesX, or neither
        val a = elseValue.get

        faX match{ // matching will help "split" faX into fa and x and then check for + or * in CASE WHEN ELSE to then create new one
          case Add(fa, x, _) => {
            if(checkConds(C, fa, x, a))
              Add(CaseWhen(Seq((C,fa)), a),
                  CaseWhen(Seq((C,x)),  Literal(0)))
            else
              cw
          }

          case Multiply(fa, x, _) => {
            if(checkConds(C, fa, x, a))
              Multiply(CaseWhen(Seq((C,fa)), a),
                       CaseWhen(Seq((C,x)),  Literal(1)))
            else
              cw
          }

          case _ => cw // operation between fa and X was neither addition or multiplication. Return normal plan with no changes
        }
      }
    }
